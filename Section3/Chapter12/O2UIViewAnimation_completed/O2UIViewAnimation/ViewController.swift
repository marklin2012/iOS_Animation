...
//            self.loginBtn.backgroundColor = UIColor(red: 0.85, green: 0.83, blue: 0.45, alpha: 1)
        ...
        view.layer.insertSublayer(balloon, above: usernameField.layer)
        
        let flight = CAKeyframeAnimation(keyPath: "position")
        flight.duration = 12
        flight.values = [CGPoint(x: -50, y: 0), CGPoint(x: view.frame.width + 50, y: 160), CGPoint(x: -50, y: loginBtn.center.y)].map {
            NSValue(CGPoint: $0)
        }
        
        flight.keyTimes = [0, 0.5, 1]
        balloon.addAnimation(flight, forKey: nil)
        balloon.position = CGPoint(x: -50, y: loginBtn.center.y)
    }
    ...
    func resetForm() {
        ...
        headingLabel.layer.addAnimation(wobble, forKey: nil)
